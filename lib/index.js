...
class StoreModel {
  constructor(parent, options) {
    ...
    try {
      ...
      const isParent = parent instanceof this.instance;
      if (!options && !isParent) {
  ...
  doc(name) {
    ...
    if (this.id && this.docType === TYPE_COLLECTION) {
      ...
      const obj = new this.instance(this, { type: TYPE_DOCUMENT, name });
  ...
  collection(name) {
    ...
    if (this.id && this.docType === TYPE_DOCUMENT) {
      ...
      name = pathNames[0];
      const obj = new this.instance(this, { type: TYPE_COLLECTION, name });
